...
const datasets = [
  {
    ...
    purpose:
      "Study facial attributes such as age distribution, skin tone variation, and facial structure differences.",
    usage:
      "Statistical distributions of facial attributes guided synthetic generation of symmetry_score and brightness_variance.",
    features: ["symmetry_score", "brightness_variance"],
  },
  {
    name: "CelebA Dataset",
    purpose:
      "Large-scale dataset containing facial attributes and landmark annotations.",
    usage:
      "Provided statistical reference for facial landmark distributions and eye region features.",
  ...
  },
  {
    ...
    usage:
      "Guided generation of pitch variation and speech stability patterns.",
  ...
  },
  {
    ...
    usage:
      "Simulated correlations between fatigue, stress, and risk prediction.",
...
export default function Dashboard() {
  ...
  const scanQuality = useMemo(() => {
    // face values are 0-1, convert to percentage
    const faceAlign = Math.min(100, Math.round(face.symmetry_score * 100 + 10));
    const lighting = Math.round((1 - face.brightness_variance) * 100);
    const audioClarity = Math.round((1 - voice.pitch_instability) * 100);
    return {
      ...
      overall: Math.max(
        0,
        Math.min(100, Math.round((faceAlign + lighting + audioClarity) / 3)),
      ),
    };
  }, [face, voice]);
  ...
  const recommendations = useMemo(() => {
    ...
    if (face.face_fatigue > 0.5) {
      recs.push({
        title: "Address Fatigue Indicators",
        text: `Facial fatigue index at ${Math.round(face.face_fatigue * 100)}%. Consider improving sleep schedule and taking regular breaks during screen time.`,
        color: "bg-amber-500",
        icon: Zap,
      });
    }
    if (voice.voice_stress > 0.5) {
      recs.push({
        title: "Manage Vocal Stress",
        text: `Voice stress detected at ${Math.round(voice.voice_stress * 100)}%. Practice breathing exercises and ensure proper hydration.`,
        color: "bg-amber-500",
        icon: Mic,
      });
    }
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="glass-card p-6 rounded-2xl">
            <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground mb-5 flex items-center gap-2">
              <SmilePlus className="w-4 h-4 text-primary" /> Facial Biomarker
              Analysis
            </h3>
            <div className="space-y-4">
              {[
                {
                  label: "Eye Stability",
                  value: Math.round((1 - face.blink_instability) * 100),
                  icon: Eye,
                  color: "from-blue-500 to-blue-400",
                },
                {
                  label: "Facial Symmetry",
                  value: Math.round(face.symmetry_score * 100),
                  icon: SmilePlus,
                  color: "from-emerald-500 to-emerald-400",
                },
                {
                  label: "Fatigue Index",
                  value: Math.round((1 - face.face_fatigue) * 100),
                  icon: Zap,
                  color: "from-amber-500 to-amber-400",
                },
                {
                  label: "Skin Brightness Score",
                  value: Math.round((1 - face.brightness_variance) * 100),
                  icon: Sun,
          ...
          <div className="glass-card p-6 rounded-2xl">
            ...
            <div className="space-y-4">
              {[
                {
                  label: "Voice Stress",
                  value: Math.round((1 - voice.voice_stress) * 100),
                  icon: Mic,
                  color: "from-rose-500 to-rose-400",
                },
                {
                  label: "Breathing Pattern",
                  value: Math.round((1 - voice.breathing_score) * 100),
                  icon: Wind,
                  color: "from-cyan-500 to-cyan-400",
                },
                {
                  label: "Pitch Stability",
                  value: Math.round((1 - voice.pitch_instability) * 100),
                  icon: Music2,
            ...
            <div className="mt-6 p-3 rounded-xl bg-muted/50 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold">
                  {Math.round(voice.voice_risk_score)}/100
                </p>
              </div>
            </div>
          </div>
      ...
      <motion.div {...fadeIn(0.2)}>
        ...
        {prediction.drift_warning && (
          <div className="mt-4 p-4 rounded-xl bg-amber-50 border border-amber-200 text-amber-800 text-sm flex items-center gap-2">
            ...
            <strong>Feature Drift Detected:</strong> Some biomarker values are
            outside the model&apos;s training distribution. Results may be less
            reliable.
          </div>
        )}
      </motion.div>
          ...
          <div className="glass-card p-6 rounded-2xl lg:col-span-2">
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-lg font-bold flex items-center gap-2">
                <BarChart3 className="w-5 h-5 text-primary" /> Feature
                Contribution
              </h3>
            </div>
            {primaryIndicator && (
            ...
            {[
              {
                ...
                value: scanQuality.faceAlign,
                color: "bg-blue-500",
              },
              {
                label: "Lighting Quality",
                value: scanQuality.lighting,
                color: "bg-amber-500",
              },
              {
                label: "Audio Clarity",
                value: scanQuality.audioClarity,
                color: "bg-emerald-500",
              },
            ].map((q, i) => (
      ...
      <motion.div {...fadeIn(0.4)}>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {recommendations.map((rec, i) => (
            ...
            >
              <div
                className={`absolute left-0 top-0 bottom-0 w-1 ${rec.color}`}
              />
              <div className="flex items-start gap-3">
                <div
                  className={`p-2 rounded-lg ${rec.color === "bg-emerald-500" ? "bg-emerald-50" : rec.color === "bg-amber-500" ? "bg-amber-50" : "bg-destructive/10"}`}
                >
                  <rec.icon
                    className={`w-4 h-4 ${rec.color === "bg-emerald-500" ? "text-emerald-500" : rec.color === "bg-amber-500" ? "text-amber-500" : "text-destructive"}`}
                  />
                </div>
                <div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                ...
                <br />
                No facial images or voice recordings are stored or transmitted
                to external servers.
              </p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-primary/5 border border-primary/10">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground leading-relaxed">
                  The training dataset was synthetically generated using
                  statistical distributions inspired by real biomedical
                  datasets. This approach ensures privacy while maintaining
                  scientifically grounded feature correlations. For example:
                  higher fatigue + higher voice stress → higher probability of
                  elevated risk; higher facial symmetry + stable pitch → lower
                  predicted risk. Gaussian noise was applied to add realistic
                  variability.
                </p>
              </div>
            </div>
        ...
        <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200/50 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-amber-700 leading-relaxed">
              ...
              <strong>AI Preventive Health Screening</strong> only. It does not
              provide medical diagnosis, treatment advice, or replace
      ...
      <footer className="py-8 border-t border-black/5 flex flex-col md:flex-row items-center justify-between text-xs text-muted-foreground gap-4">
        <div className="flex items-center gap-2">
          ...
          <span>
            Core Model v{prediction.model_version} · Risk Level:{" "}

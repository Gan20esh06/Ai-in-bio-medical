 ...
 * This ensures the backend receives a proper WAV file regardless of the
 * browser's recording codec (WebM/Opus, etc.).
 */
...
/** Decode any browser-supported audio Blob into a WAV data-URL. */
async function blobToWavDataUrl(
  blob: Blob,
): Promise<{ dataUrl: string; sampleRate: number }> {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () =>
      resolve({ dataUrl: reader.result as string, sampleRate: sr });
...
export default function VoiceScan() {
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        // Convert WebM → WAV so the backend receives a proper WAV file
        const webmBlob = new Blob(audioChunksRef.current, {
          type: "audio/webm",
        });
        try {
          const { dataUrl, sampleRate: sr } = await blobToWavDataUrl(webmBlob);
          setAudioSampleRate(sr);
          setCapturedAudio(dataUrl);
        } catch (err) {
          console.error("WAV conversion failed, sending raw:", err);
          // Fallback: send WebM as-is (backend will attempt to decode)
          const reader = new FileReader();
  ...
  const handleAudioUpload = useCallback(() => {
    ...
    input.onchange = async (e) => {
      ...
      if (!file) return;
      try {
        const { dataUrl, sampleRate: sr } = await blobToWavDataUrl(file);
        setAudioSampleRate(sr);
        setCapturedAudio(dataUrl);
      } catch {
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      >
        ...
        <p className="text-muted-foreground max-w-lg mx-auto">
          Vocal patterns reveal micro-tremors, breathing irregularities, and
          stress markers invisible to the human ear.
        </p>
      </motion.div>
      ...
      <div className="space-y-8">
        ...
        >
          ...
          <p className="text-xl md:text-2xl font-medium leading-relaxed text-foreground/90 max-w-2xl mx-auto">
            ...
            <br />I am speaking clearly for my health analysis.&rdquo;
          </p>
        </motion.div>
              ...
              <div className="absolute inset-0 flex items-center justify-center gap-0.5 pointer-events-none">
                {Array.from({ length: 50 }).map((_, i) => (
                  <motion.div
                    key={i}
                    animate={{
                      height: ["15%", `${Math.random() * 70 + 20}%`, "15%"],
                    }}
                    transition={{
        ...
        <div className="flex justify-between pt-4 border-t">
          ...
          <Button
            ...
            disabled={!capturedAudio}
            title={!capturedAudio ? "Record or upload audio first" : undefined}
          >

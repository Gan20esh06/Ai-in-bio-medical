...
def predict_health_risk(features: Dict[str, float]) -> Dict[str, Any]:
    ...
    try:
        ...
        _MIN_CONFIDENCE = 5.0  # only reject when model is effectively random
        if confidence_score < _MIN_CONFIDENCE:
            logger.warning(
                "Prediction confidence too low (%.1f%% < %.1f%%) — rejecting.",
                confidence_score, _MIN_CONFIDENCE,
            )
            return {
                "error": f"Model confidence too low ({confidence_score:.0f}%). Input data may not contain clear health signals.",
...
def _fallback_result(reason: str) -> Dict[str, Any]:
    """Return an error result when prediction cannot be performed.

    Intentionally does NOT include fake risk scores or feature contributions
    so that callers can see this is an error, not a real prediction.
    """
    logger.warning("Prediction rejected — reason: %s", reason)
    return {
        "error": reason,

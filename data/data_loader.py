...
def _load_diabetes(seed: int = 42) -> Optional[pd.DataFrame]:
    ...
    # Rename columns to expected names if the CSV has its own header
    df.columns = _PIMA_COLS[:len(df.columns)]
    # Ensure all columns are numeric (drop any rows that can't be converted)
